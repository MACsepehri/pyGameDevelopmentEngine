...
class Object:
    ...
    def collision(self, game_object: Object):
        if self.obj_type == "rect":
            rect1 = self.rect
            rect2 = game_object.rect
            return rect1.colliderect(rect2)
        elif self.obj_type == "image":
            rect1 = self.image.get_rect()
            rect2 = game_object.image.get_rect()
            return rect1.colliderect(rect2)
        else: 
...
class Window:
    def __init__(self, size=None, title="GDE", fullscreen=False, icon=None):
        ...
        self.icon = icon
        if size == None: self.win = pygame.display.set_mode((self.displayInfo.current_w, self.displayInfo.current_h))
